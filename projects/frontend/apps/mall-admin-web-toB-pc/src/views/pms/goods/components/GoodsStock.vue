				...
				<el-form ref="specFormRef" :model="specForm" :inline="true" size="small">
					<el-table ref="specTableRef" :data="specForm.specList" row-key="id" size="small">
						...
						<el-table-column>
							...
							<template #default="scope">
								<div v-for="item in scope.row.values" :key="item.id" style="display: inline-block; margin-right: 15px">
									<el-tag
										size="small"
										closable
								...
								/>
								<el-button v-else style="vertical-align: top" size="small" @click="handleSpecValueAdd(scope.$index)">
									<i-ep-plus /> 添加规格值
								</el-button>
							</template>
						</el-table-column>
						...
						<el-table-column width="60" label="操作">
							<template #default="scope">
								<el-button type="danger" size="small" circle plain @click.stop="handleSpecRemove(scope.$index)"
									><i-ep-minus />
								</el-button>
							</template>
						</el-table-column>
